...
public class SeansActionController : Controller
{
    ...
    [HttpGet]
    public JsonResult seansGuncelToplamlar()
    {
            try
            {
                using(var context = new U1710750DbCb9Context())
                {
                    var guncelSeansKayit = context.SeansOgrSets.Where(x => x.Seans.Tarih.Date >= DateTime.Now.Date && x.Ogr.KutuphaneId == (int)HttpContext.Session.GetInt32("kutuphaneID")).Select(s => new SeansOgrSet()
                    {
                        Id = s.Id,
                        Seans = s.Seans,
                        Durum = s.Durum,
                    ...
                    // Güncel Ön kayıtlar
                    int bekleyenOnKayitToplam = 0;
                    bekleyenOnKayitToplam = guncelSeansKayit.Where(a => a.Durum == 0).Count();
                    double bekleyenOnKayitToplamCiro = 0;
                    bekleyenOnKayitToplamCiro = guncelSeansKayit.Where(a => a.Durum == 0).Sum(t => t.Seans.SeansUcret);
                    // Güncel Aktif Kayıtlar
                    int aktifSeansKayitToplam = 0;
                    aktifSeansKayitToplam = guncelSeansKayit.Where(a => a.Durum != 0).Count();
                    double aktifSeansKayitToplamCiro = 0;
                    aktifSeansKayitToplamCiro = guncelSeansKayit.Where(a => a.Durum != 0).Sum(t => t.Seans.SeansUcret);
                    // Güncel Seanslar
                    int guncelSeansToplami = 0;
                    guncelSeansToplami = context.DenemeSeanslars.Where(x => x.Deneme.Kategori.Kat.KutuphaneId == (int)HttpContext.Session.GetInt32("kutuphaneID") && x.Tarih.Date >= DateTime.Now.Date).Count();
                    MesajBaslik = "İşlem Başarılı";
                    Mesaj = "Güncel Toplamlar Hesaplandı";
                    MesajIcon = "success";
                    return Json(new { MesajBaslik, Mesaj, MesajIcon, bekleyenOnKayitToplam, bekleyenOnKayitToplamCiro, aktifSeansKayitToplam, aktifSeansKayitToplamCiro, guncelSeansToplami });
    ...
    [HttpPost]
    public JsonResult seansOgrCopy(int seansid, int newseansid)
    {
        try
        {
            using(var context = new U1710750DbCb9Context())
            {
                    var seansogrlist = context.SeansOgrSets.Where(x => x.SeansId == seansid).Select(s => new SeansOgrSet()
                    {
                        OgrId = s.OgrId,
                        SeansId = s.SeansId,
                        SeansKayitTarih = DateTime.Now,
                        Durum = s.Durum
                    }).ToList();

                    foreach (var item in seansogrlist)
                    {
                        var ogrofseanscopy = new SeansOgrSet();
                        ogrofseanscopy.OgrId = item.OgrId;
                        ogrofseanscopy.SeansId = newseansid;
                            ogrofseanscopy.SeansKayitTarih = item.SeansKayitTarih;
                        ogrofseanscopy.Durum = item.Durum;
                        context.SeansOgrSets.Add(ogrofseanscopy);
                    }
                    context.SaveChanges();
                    MesajBaslik = "İşlem Başarılı";
                    Mesaj = "Öğrenciler Seçilen Seansa Kopyalandı";
                    MesajIcon = "success";
                    return Json(new { MesajBaslik, Mesaj, MesajIcon });
        ...
        try
        {
            ...
            using (var context = new U1710750DbCb9Context())
            {
                ...
                if (seanskontrol != null)
                {
                    ...
                    var sninf = context.DenemeSeanslars.Where(x => x.Id == x.SeansOgrSets.Where(x => x.Id == seanskayitno || x.Qr == qrcode).Select(s => s.SeansId)
                            .First()).Select(s => new viewSeansModel()
                        {
                            ...
                            TarihSTR = s.Tarih.ToString("dd.MM.yyyy - dddd"),
                            Saat = s.Saat,
                            qrCodeforOgr = s.SeansOgrSets.Where(t => t.OgrId == ogrid).Select(s => s.Qr).First()
                        }).First();
    ...
    public JsonResult seanschange(int snsetid, int yeniseansid)
    {
        ...
        using (U1710750DbCb9Context context = new U1710750DbCb9Context())
        {
            try
            {
    ...
    public JsonResult multipushnotification(int type, int seansid, int kategoriid, string baslik, string mesaj)
    {
        ...
        try
        {
            ...
            using (var context = new U1710750DbCb9Context())
            {
                ...
                {
                    using (var contexte = new U1710750DbCb9Context())
                    {
