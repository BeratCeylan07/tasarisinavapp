...
public IActionResult OgrListExcel()
{
    try
    {
        using (var workbook = new XLWorkbook())
        {
            var ogrexcel = workbook.Worksheets.Add("Öğrenci Listesi");
            ogrexcel.Cell(1, 1).Value = "Ad Soyad";
            ogrexcel.Cell(1, 2).Value = "Kategori";
            ogrexcel.Cell(1, 3).Value = "Telefon";
            ogrexcel.Cell(1, 4).Value = "Sınıf";
            ogrexcel.Cell(1, 5).Value = "Okul";
            int ogrRowcount = 2;
            foreach (var item in ogrListforExcel())
            {
                ogrexcel.Cell(ogrRowcount, 1).Value = item.Ad + " " + item.Soyad;
                ogrexcel.Cell(ogrRowcount, 2).Value = item.Kategori.AnaKategoriBaslik;
                ogrexcel.Cell(ogrRowcount, 3).Value = item.Telefon;
                ogrexcel.Cell(ogrRowcount, 4).Value = item.Sinif;
                ogrexcel.Cell(ogrRowcount, 5).Value = item.OkulSTR;
                ogrRowcount++;
            }
            ...
            using (var stream = new MemoryStream())
            {
                ...
                return File(content, "application/vdn.openxmlformats-officedocument.spreadsheetml.sheet",$"OgrenciListesi_{DateTime.Now}.xlsx");
            }
        }
    }
    catch (Exception ex)
...
public List<viewOgrencilerModel> ogrListforExcel()
{
    try
    {
        using (var context = new U1710750DbCb9Context())
        {
            ...
            List<viewOgrencilerModel> ogrlist = context.Ogrencilers.Where(x => x.KutuphaneId == kutuphaneid).Select(s =>
                new viewOgrencilerModel
                {
                    ...
                    Sinif = s.Sinif,
                    OkulSTR = s.Okul,
                }).OrderByDescending(o => o.Kategori.AnaKategoriBaslik).ToList();
