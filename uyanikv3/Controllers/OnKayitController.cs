...
public class OnKayitController : Controller
{
    ...
    public IActionResult OgrLogin(int? sonuc)
    {
    ...
    public JsonResult ogrSeansGetir(viewOgrencilerModel model)
        {
            using (var db = new U1710750DbCb9Context())
            {
                try
                {
    ...
    {
        Ogrenciler controlModel = ogrControl(model.Telefon);
        if (controlModel is not null)
        {
            if (controlModel.Ad == model.Ad && controlModel.Soyad == model.Soyad)
            {
                HttpContext.Session.Clear();
                ...
                HttpContext.Session.SetString("OgrOkul",model.OkulSTR ?? "");
                HttpContext.Session.SetString("OgrSinif",model.Sinif ?? "");
                return RedirectToAction("Index");
            }
            else
            {
                return RedirectToAction("OgrLogin", new { sonuc = 0 });
            }
    ...
    public JsonResult SeansOnKayit(viewOgrencilerModel model)
    {
        ...
        string soyad = HttpContext.Session.GetString("OgrSoyad");
        string tel = HttpContext.Session.GetString("OgrTel");
        string sinif = HttpContext.Session.GetString("OgrSinif");
        string okul = HttpContext.Session.GetString("OgrOkul");
        try
        {
                        using (var context = new U1710750DbCb9Context())
                        {
    ...
    Ogrenciler ogrControl(string telno)
    {
        try
        {
            using (var context = new U1710750DbCb9Context())
            {
                var ogr = context.Ogrencilers.Where(x => x.Telefon == telno).First();
                if (ogr is not null)
                {
                    return ogr;
                }else
